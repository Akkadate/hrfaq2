...
function loadViewData(viewName) {
    ...
    // หน่วงเวลาเล็กน้อยเพื่อให้ loading แสดง
    setTimeout(() => {
        try {
            switch (viewName) {
                case 'dashboard':
...
function goToPage(pageNumber) {
    if (pageNumber < 1 || pageNumber > ADMIN_UI_STATE.pagination.totalPages) {
        return;
    }
...
function setupUIEventListeners() {
    ...
    if (categoryFilter) {
        categoryFilter.addEventListener('change', function() {
            ...
            ADMIN_UI_STATE.pagination.currentPage = 1; // รีเซ็ตหน้าเป็นหน้าแรก
            loadQuestionsData();
        });
    }
    ...
    if (questionSearch && questionSearchBtn) {
        ...
        // ล้างการค้นหาเมื่อเคลียร์ช่องค้นหา
        questionSearch.addEventListener('input', function() {
            if (this.value.trim() === '' && ADMIN_UI_STATE.filters.searchQuery !== '') {
                ADMIN_UI_STATE.filters.searchQuery = '';
                ADMIN_UI_STATE.pagination.currentPage = 1; // รีเซ็ตหน้าเป็นหน้าแรก
                loadQuestionsData();
            }
        });
